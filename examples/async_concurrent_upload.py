...
SERVERS = ["https://blossom.band", "https://nostr.download", "https://blossom.primal.net"]
...
async def main():
    ...
    results = await client.upload_to_all(
        data=test_data, mime_type="text/plain", description="Concurrent upload test", use_auth=True
    )
...
async def compare_with_sequential():
    ...
    start = time.time()
    sync_results = client_sync.upload_to_all(data=test_data, mime_type="text/plain", use_auth=True)
    ...
    async_results = await client_async.upload_to_all(
        data=test_data, mime_type="text/plain", use_auth=True
    )
